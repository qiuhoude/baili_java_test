...
public class Lesson7_1 {
    ...
    /**
     *
     * @param horses 使用函数的递归（嵌套）调用，找出所有可能的马匹出战顺序
     * @param result 目前还剩多少马没有出战，result-保存当前已经出战的马匹及顺序
     */
    public static void permutate(ArrayList<String> horses, ArrayList<String> result) {
        ...
        for (int i = 0; i < horses.size(); i++) {
            ...
            // 将已选择的马匹从未出战的列表中移出
            ArrayList<String> res_horses = ((ArrayList<String>) horses.clone());
            res_horses.remove(i);
            // 递归调用，对于剩余的马匹继续生成排列
            permutate(res_horses, new_result);
        }
    }
...
//        ArrayList<String> horses = new ArrayList<>(Arrays.asList("q1", "q2", "q3"));
